...
#KG 2015-10-6
epicsEnvSet("P",         "XF:23ID1-ES")
epicsEnvSet("TP_PORT",   "P0")
# epicsEnvSet("IOCNAME",   "mc14")
#epicsEnvSet("IOC_PREFIX", "$(P){IOC:$(IOCNAME)}")
epicsEnvSet("MC",   "MC:12")
epicsEnvSet("CT",   "XF:23ID1-CT")
...
pmacAsynIPConfigure("P0","xf23id1-mc12:1025")
...
pmacCreateController("motor","P0",0,9,100,100)
...
pmacDisableLimitsCheck("motor", 7, 0)
pmacDisableLimitsCheck("motor", 8, 0)
...
dbLoadRecords("db/asynComm.db","P=XF:23ID1-CT{MC:12},PORT=P0,ADDR=0")
dbLoadTemplate("db/motorstatus.substitutions")
dbLoadRecords("db/pmacStatus.db","SYS=XF:23ID1-CT,PMAC=MC:12,VERSION=1,PLC=5,PORT=P0,NAXES=8")
...
dbLoadRecords("$(EPICS_BASE)/db/iocAdminSoft.db","IOC=XF:23ID1-CT{IOC:MC12}")
dbLoadRecords("db/cryoangle.db")
dbLoadRecords("$(TOP)/iocBoot/$(IOC)/read_var.db","P=$(P){IOC-$(MC)},R=M1Enc,PORT=$(TP_PORT),ADDR=0,VAR=M8000,SCALE=1.0,OFFSET=0.0")
dbLoadRecords("$(TOP)/iocBoot/$(IOC)/read_var.db","P=$(P){IOC-$(MC)},R=M2Enc,PORT=$(TP_PORT),ADDR=0,VAR=M8001,SCALE=1.0,OFFSET=0.0")
dbLoadRecords("$(TOP)/iocBoot/$(IOC)/read_var.db","P=$(P){IOC-$(MC)},R=M3Enc,PORT=$(TP_PORT),ADDR=0,VAR=M8002,SCALE=1.0,OFFSET=0.0")
dbLoadRecords("$(TOP)/iocBoot/$(IOC)/read_var.db","P=$(P){IOC-$(MC)},R=M4Enc,PORT=$(TP_PORT),ADDR=0,VAR=M8003,SCALE=1.0,OFFSET=0.0")
dbLoadRecords("$(TOP)/iocBoot/$(IOC)/read_var.db","P=$(P){IOC-$(MC)},R=M5Enc,PORT=$(TP_PORT),ADDR=0,VAR=M8004,SCALE=1.0,OFFSET=0.0")
...
dbl > "/cf-update/xf23id1-ioc3.mc12.dbl"
